...
from ffmp_conv import ffmpeg_recursive
...
required_options = (
    'NZBPO_FROM', 'NZBPO_TO', 'NZBPO_SERVER', 'NZBPO_PORT', 'NZBPO_ENCRYPTION', 'NZBPO_USERNAME', 'NZBPO_PASSWORD')
...
# run post processing
try:
    for f in str(text).splitlines():
        try:
            r_code, new_name = ffmpeg_recursive.main(op_args=f)
            if r_code == 0:
                if new_name is not None:
                    print(f'[NZB] DIRECTORY={new_name}')
